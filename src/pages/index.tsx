...
export default function Home() {
    ...
    // ? Search users
    async function sendSearch(e: FormEvent) {
        e.preventDefault();
        setLoading(true);
        try {
                ...
                }
                {
                    ...
                    <div className="w-full mb-6 h-min st:h-72 px-8 overflow-y-auto line-center mt-6 items-start flex-wrap gap-2 overflow-x-hidden">
                        {users.map((user: any) => (
                            <div
                                key={user.id}
                                className="w-11/12 line-left gap-x-4 border shadow-md h-12 hover:bg-zinc-300 cursor-pointer mt-2 text-xs rounded-xl px-8 with-transition"
