...
export default function User({user}){
    ...
    // ? add more commits
    async function addContent(page){
        setLoadingMore(true)
        try{
            const response = await api.get(`/users/${user}/repos?per_page=100&page=${page+1}`)
            console.log('response->', response.data)
                    ...
                    <>
                        ...
                        }
                        {
                            ...
                            <div className='w-full  cursor-pointer border-2 px-8 py-4 text-zinc-800 border-zinc-300 hover:bg-zinc-300 hover:text-white line-center  with-transition'>
                                <ReactLoading type='spinningBubbles' height={50} width={50} color='#1E282C'/>
                            </div>
                        }
                        {
                            // * close line
