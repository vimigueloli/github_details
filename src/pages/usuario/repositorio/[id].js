    ...
    useEffect(() => {
        async function getUser(){
            if(user === ''  || user === null){
                ...
                setLoading(true)
                try{
                    const response = await api.get(`/repos/${user}/${repo}/branches`)
                    const userResponse = await api.get(`/repos/${user}/${repo}`)
                    setBranch(response.data)
                    setRepoData(userResponse.data)
                    setLoading(false)
                }catch (e){
                    toast.error('Houve algum erro ao tentar visualizar os repositórios de '+user)
                    router.back()
                }
            }
        }
                ...
                }
                {
                    ...
                    <>
                        <div className='w-full h-16 line-center'>
                            <div className='text-2xl font-bold text-zinc-800'>{repo}</div>
                        </div>
                        <div className='line-center'>
                            <div className='w-80 text-lg text-center'>{repoData? repoData.description : ''}</div>
                        </div>
                        <div className='line-center'>
                            <div className='text-xl font-semibold text-zinc'>Branches</div>
                        </div>
                        {
                            // * branches
                            !selected &&
                                ...
                                <div className='w-full st:w-80 '>
                                    {
                                        ...
                                        branch.map((branch, index)=>{
                                            return(
                                                ...
                                                >
                                                    <div className='w-80 text-lg text-center'>{branch.name}</div>
                                                </div>
                                            )
                                        })
                                    }
                            ...
                            <>
                                <div className='line-center mt-8 mb-4'>
                                    ...
                                    >
                                        ...
                                        <XIcon width={25} height={25}  className='ml-auto mr-2'/>
                                    </div>
                                </div>
                                    {
                                        commits.length > 0 ?
                                        commits.map((commit)=>
                                            <div className='line-center items-start' key={commit.id}>
                                                <div className='line-between w-96'>
                                                    <div className=' line-left'>
                                                        <div className='w-4'>
                                                            <AiOutlineBranches width={25} height={25} color='#1E282C' />
                                                        </div>
                                                    ...
                                                    <div className='text-zinc-800/50 ml-3'>
                                                        {new Date(commit.commit.committer.date).getDate()}/{new Date(commit.commit.committer.date).getMonth()+1}/{new Date(commit.commit.committer.date).getFullYear()}
                                                    </div>
                                                </div>
                                            </div>
                                        )
                                        :<div className='line-center '>
                                            <ReactLoading type='spinningBubbles' color='#000' height={25} width={25} />
                                        </div>
                                    }
